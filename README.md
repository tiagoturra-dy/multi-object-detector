...
gcloud run deploy yoloe-api --source . --platform managed --memory 4Gi --cpu 2 --timeout 300 --min-instances=0 --max-instances=3 --set-env-vars="DETECT_API_KEY=$(gcloud secrets versions access latest --secret='object-detector-key')" --allow-unauthenticated
```
